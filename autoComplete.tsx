...
const AutoComplete = () => {
  ...
  const createDependencyProposals = [
    ...
    {
      label: '"author"',
      description: 'include firstName and lastName',
      insertText: 'author',
      children: [
        {
          label: '"first name"',
    ...
    if (monaco) {
      const provider = monaco.languages.registerCompletionItemProvider('handlebars', {
        ...
        provideCompletionItems: (model, position) => {
          const suggestions = buildSuggestions(monaco, createDependencyProposals, model, position);
          return {
            suggestions,
      ...
      return function cleanup() {
        //depose the languages auto complete here!
        provider.dispose();
      };
    }
  }, [monaco]);
  ...
  return (
    <div>
      <label> Handlebars Auto Completed</label>
      <MonacoEditor height={200} value="" language="handlebars" theme="vs-dark" />
    </div>
  );
};
